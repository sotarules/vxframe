...
RecordImporter = {
    ...
    importFileParse(uploadStats, messages) {
        ...
        let fut, readStream, stream, abort, read, processed, total, readPercentOld, readPercentNew, rowArray,
            ...
            uploadChunkSize, indexStart, modifier;
        ...
        try {
            ...
            streamQueue = async.queue(Meteor.bindEnvironment((task, callback) => {
                try {
                    ...
                    result = RecordImporter.importFileChunk(uploadStats, task.rowArray, messages, task.name)
                    if (!result.success) {
                        fut.return(result)
                        return
                    }
     ...
     * @return {object} Result object.
     */
    importFileChunk(uploadStats, rowArray, messages, taskName) {
        try {
            ...
            _.each(rowArray, rowObject => {
                // Standard CSV Import format on Sherpa uses the first row as a heading:
                if (rowObject.index === 0) {
                    headerArray = rowObject.row
                    OLog.debug(`recordimporter.js importFileChunk domainId=${uploadStats.domain} ` +
                ...
                try {
                    ...
                    func(uploadStats, uploadTypeObject, headerArray, rowObject.row, rowObject.index, rowObject.rowIndex,
                        messages, insertArray)
                }
                catch (error) {
    ...
    performSimpleUpsert(coll, uploadStats, taskName, insertArray) {
        insertArray.forEach(record => {
            OLog.debug(`recordimporter.js performSimpleUpsert domainId=${uploadStats.domain} ` +
                `uploadType=${uploadStats.uploadType} userId=${uploadStats.userId} taskName=${taskName} ` +
                `selector=${OLog.debugString(record.selector)} modifier=${OLog.debugString(record.modifier)}`)
            coll.upsert(record.selector, record.modifier);
        })
    },
     ...
     * @param {obecjt} uploadTypeObject Upload type object from codes.
     ...
     */
    prepareGeneric(uploadStats, uploadTypeObject, headerArray, row, index,
        rowIndex, messages, insertArray) {

        const keypathIndex = headerArray.indexOf(uploadTypeObject.keypath)

        //OLog.debug(`recordimporter.js prepareGeneric uploadType=${uploadStats.uploadType} ` +
        //    `keypath=${uploadTypeObject.keypath} keyPathIndex=${keypathIndex} ` +
        //    `index=${index} rowIndex=${rowIndex} row=${row}`)

        const record = {}

        record.selector = {}
        record.selector.domain = uploadStats.domain
        record.selector[uploadTypeObject.keypath] = row[keypathIndex]

        record.modifier = {}
        record.modifier.$set = {}
        record.modifier.$set.dateCreated = new Date()
        record.modifier.$set.userCreated = uploadStats.userId
        record.modifier.$set.dateModified = new Date()
        record.modifier.$set.userModified = uploadStats.userId
        record.modifier.$set.domain = uploadStats.domain
        ...
        headerArray.forEach((path, columnIndex) => {
            ...
            const value = FX.trim.strip(row[columnIndex])
            valid &= VXApp.validate(value, VX.common.name, index, messages, fieldIdKey, fieldIdVariables)
            set(record.modifier.$set, path, value)
        })

        if (!valid) {
            OLog.debug(`recordimporter.js prepareGeneric *bypassing* index=${index} valid=${valid} ` +
                `messages=${OLog.debugString(messages)}`)
            return
        }

        insertArray.push(record)
    },
