...
Reporter = {
    ...
    run : () => {
        ...
        if (!Util.getConfigValue("enableReporter")) {
            //OLog.debug("reporter.js reconcile *disabled* system administrator has disabled reporter system-wide")
            return
        }
    ...
    createScheduledReports: (user) => {
        ...
        user.profile.reportPreferences.forEach((reportPreference, index) => {
            ...
            var reportParameterDefaults, nextDate, $set, result
            ...
            try {
                ...
                reportParameterDefaults = Util.reportParameterDefaults(reportPreference.reportType, user._id)

                result = Reporter.sendReport(user._id, reportPreference.reportType, reportParameterDefaults)
                if (!result.success) {
                    return
                }

                nextDate = Util.computeNextDate(reportPreference.reportFrequency, reportPreference.timeUnit,
                ...
                OLog.debug("reporter.js createScheduledReports send completed, updating profile email=" + Util.getUserEmail(user._id) + " $set=" + OLog.debugString($set))
    ...
    sendReport : (recipientId, reportType, reportParameters) => {
        ...
        var recipient, reportName, reportTypeObject, templateName, data, send

        try {

            if (!(_.isString(recipientId) && _.isString(reportType) && (!reportParameters || _.isObject(reportParameters)))) {
                OLog.error("reporter.js sendReport parameter check failed recipientId=" + recipientId + " reportType=" + reportType +
            ...
            reportName = Util.i18n("codes.reportType." + reportType)
            reportTypeObject = Meteor.i18nMessages.codes.reportType[reportType]
            templateName = reportTypeObject.template_name

            data = Reporter.getDataReal(reportType, recipient, reportParameters)

            send = {}
            send.to = Util.getUserEmail(recipientId)
            send.subject = Util.i18n("reporter.label_mail_subject", { reportName: reportName, adminName : Util.fetchFullName(recipientId),
            ...
            OLog.debug("reporter.js sendReport recipientId=" + recipientId + " reportType=" + reportType + " templateName=" + templateName +
             " send=" + OLog.debugString(send) + " MAIL_URL=" + process.env.MAIL_URL)
