...
export default class UserEditRight extends Component {
    ...
    updateEmail(component, strippedValue) {
        ...
        OLog.debug("UserExitRight.jsx updateEmail strippedValue=" + strippedValue  + " modifier=" + OLog.debugString(modifier))
        Meteor.users.update(this.props.user._id, modifier, (error) => {
            if (error) {
                OLog.error("UserExitRight.jsx updateEmail error returned from dynamic field update=" + error)
                UX.notifyForDatabaseError(error)
                return
            }
            OLog.debug("UserExitRight.jsx updateEmail *success*")
        })
    }
